...
async function validateAndTransformRecord<
  TransformedKey extends string | number,
  TransformedValue,
>({
  value,
  isKeyNumeric,
  ...
  transformKey: (
    key: string | number,
  ) => MaybePromise<MaybeValid<TransformedKey>>;
  transformValue: (
    value: unknown,
    key: string | number,
  ) => MaybePromise<MaybeValid<TransformedValue>>;
  ...
  >(
    ...
    Promise.resolve({
      ok: true,
      value: {} as Record<TransformedKey, TransformedValue>,
    }),
  );
}
